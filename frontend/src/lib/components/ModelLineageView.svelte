<script lang="ts">
	...
	async function load() {
		loading = true;
		try {
			models = await listRegistryModels();
		} catch (e) {
	...
	const { nodes, edges } = $derived.by(() => {
		...
		const ns: Node[] = models.map((m) => ({
			...
			data: {
				label: m.name,
				stage: (m.stage as ModelStage) ?? 'draft',
	...
	async function save(event: Event) {
		...
		busy = true;
		try {
			await updateRegistryModel(selectedId, form);
	...
	async function remove() {
		...
		busy = true;
		try {
			await deleteRegistryModel(selected.id);
			selectedId = null;
			await load();
	...
	<div class="flow-shell" class:drawer-open={selectedId !== null}>
			<div class="flow-canvas">
				...
				{:else if models.length === 0}
					<div class="empty">
						<p class="eyebrow">Empty lineage</p>
						<h2>Register a model to start a tree.</h2>
						<p class="muted">
							Register a model in the Registry view, then set parents here and watch the lineage
							take shape.
						</p>
					</div>
				{:else}
			...
			{#if selected}
				<aside class="drawer">
					...
					<dl class="kv">
						<dt>Stage</dt><dd><span class="stage-chip" data-stage={selected.stage}>{stageName(selected.stage)}</span></dd>
